...
export async function generateMealPlanV3(options: GeneratePlanOptions) {
  ...
  const allRecipes: Recipe[] = recipesRaw.map((r: any) => ({
    ...
    makes_leftovers: !!r.makes_leftovers,
  ...
  return await transaction(async (client) => {
    ...
    console.log(`[mealPlanGeneratorV3] Plan ${mealPlanId} (${existing ? "existing" : "new"}), cleared ${deleteResult.rowCount} old items`);
    ...
    for (const daySchedule of cookingSchedule) {
      ...
      if (daySchedule.meal_mode === "one_main") {
        ...
        if (recipe) {
          ...
          // Add sides
          const sides = await selectSidesForMain(client, recipe, 1);
        ...
        if (!assignments || assignments.length === 0) {
          ...
          assignments = Array.from({ length: numMains }, (_: any, i: number) => ({
            main_number: i + 1,
        ...
        for (const assignment of assignments) {
          ...
          const assignedMembers = members.filter((m: any) =>
      ...
      if (lunchNeed.leftovers_ok) {
        ...
        if (prevDayIndex >= 0) {
          ...
          const prevDayMeal = plannedMeals.find(
            (m: any) => m.day === prevDay && m.meal_type === "main"
          );
    ...
    console.log("[mealPlanGeneratorV3] All planned meals:", plannedMeals.map((m: any) => `${m.day}/${m.meal_type}/${m.recipe_id}`));
...
// Helper: Find recipes matching a keyword description
  ...
  for (const recipe of recipes) {
    ...
    if (score === 0 && foodWords.length > 0) {
      ...
      for (const word of foodWords) {
        ...
        if (ingredientNames.some((name: string) => name.includes(word))) {
          ingredientHits++;
        }
        ...
        if (relatedTerms[word]) {
          for (const related of relatedTerms[word]) {
            if (nameLower.includes(related) || proteinLower.includes(related) ||
                ingredientNames.some((name: string) => name.includes(related)) ||
    ...
    if (score === 0 && foodWords.length <= 1) {
      ...
      if (proteinLower.includes(singleWord)) {
        ...
        score = 80;
      } else if (ingredientNames.some((name: string) => name.includes(singleWord))) {
        score = 50;
      }
      ...
      if (score === 0 && relatedTerms[singleWord]) {
        for (const related of relatedTerms[singleWord]) {
          ...
          if (ingredientNames.some((name: string) => name.includes(related))) {
            score = 30;
            break;
          }
        }
...
function shouldBeVegetarianDay(
  ...
  const targetVegCount = Math.round((vegetarianRatio / 100) * 7);
...
// Helper: Select sides for a main (uses client for transaction context)
async function selectSidesForMain(client: any, mainRecipe: Recipe, count: number = 1): Promise<any[]> {
